...
const updateResponseEditor = (value) => {
  if (!responseEditor) {
    responseEditor = CodeMirror(jsonResponseBody, {
      lineNumbers: true,
      tabSize: 2,
    ...
    return;
  } else responseEditor.setValue(JSON.stringify(value, null, 2));
};

urlInput.addEventListener("blur", () => {
  const value = {cache: "no"}
  if (!requestEditor) {
    requestEditor= CodeMirror(jsonRequestBody, {
      lineNumbers: true,
      tabSize: 2,
      value: JSON.stringify(value, null, 2),
      mode: "json",
    ...
    return;
  } else requestEditor.setValue(JSON.stringify(value, null, 2));
})
...
form.addEventListener("submit", (e) => {
  ...
  let data;
  try {
    data = JSON.parse(requestEditor.getValue() || null, 2);
  } catch (e) {
